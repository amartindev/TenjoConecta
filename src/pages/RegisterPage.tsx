...
export function RegisterPage() {
  ...
  const categories: string[] = Array.isArray(CATEGORIES) ? CATEGORIES : Object.values(CATEGORIES);
    ...
    try {
      ...
      if (selectedFiles.length > 0 && business) {
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          const {
            data: { publicUrl },
          } = supabase.storage.from("businessimages").getPublicUrl(filePath);

          const { error: imageError } = await supabase.from("business_images").insert({
      ...
      if (selectedPdf && business) {
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from("businesspdf").getPublicUrl(filePath);

        const { error: pdfError } = await supabase.from("business_pdf").insert({
          business_id: business.id,
          url: publicUrl,
          storage_path: filePath,
        });
